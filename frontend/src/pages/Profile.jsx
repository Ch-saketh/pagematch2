...
const Profile = () => {
  ...
  // Developer architectural avatar identifiers
  ...
  useEffect(() => {
    ...
    // Sanitize any legacy Unsplash images from stored profiles
    let needsResave = false;
    stored = stored.map((p, idx) => {
      if (typeof p.avatar === 'string' && p.avatar.includes('unsplash.com')) {
        needsResave = true;
    ...
    if (stored.length === 0) {
      ...
      needsResave = true;
      localStorage.setItem('selectedProfile', initialProfile.id);
    }
